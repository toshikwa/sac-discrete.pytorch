...
class QNetwork(BaseNetwork):
    ...
    def __init__(self, num_actions, input_channels=1):
...
class DiscretePolicy(BaseNetwork):
    ...
    def sample(self, state):
        ...
        max_prob_actions = torch.argmax(action_probs).unsqueeze()
        # distribution of policy's actions
        action_dist = Categorical(action_probs)
        # sample actions
        actions = action_dist.sample().cpu()
        # log likelihood of actions
        log_action_probs = torch.log(torch.min(action_probs, 1e-8))

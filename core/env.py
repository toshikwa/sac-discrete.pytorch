...
class NoopResetEnv(gym.Wrapper):
    ...
    def reset(self, **kwargs):
        ...
        assert noops > 0
        # repeat no-op action to reset env
        obs = None
...
def wrap_deepmind(env, episode_life=True, clip_rewards=False, frame_stack=True,
                  scale=True):
    """
    ...
    :return: (Gym Environment) the wrapped atari environment
    """
    # consider 'loss of life' as termination
    if episode_life:
...
class PyTorchEnv(gym.ObservationWrapper):
    def __init__(self, env):
        ...
        self.observation_space = spaces.Box(
            low=0, high=1.0,
            shape=self._shape(env.observation_space.shape),
            dtype=np.float32)
    ...
    def observation(self, observation):
        return self._transpose(observation)

    def _transpose(self, x):
        if isinstance(x, np.ndarray):
            return np.transpose(x, (2, 0, 1))
        elif isinstance(x, LazyFrames):
            return np.transpose(np.array(x, np.float32), (2, 0, 1))


def make_pytorch_env(env_id, wrapper_kwargs=None):
